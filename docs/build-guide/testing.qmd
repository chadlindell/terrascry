...
|-----------|---------|----------------------|
| Power Supply | Probe power delivery | 5V or 12V, current-limited to 2A **(Target)** |
| Digital Multimeter | Resistance, voltage, current | 0.1% accuracy **(Target)**, 4.5 digit |
| LCR Meter | Coil inductance and Q factor | 10 kHz test frequency, 0.1% accuracy **(Target)** |
| Oscilloscope | Signal verification | 50 MHz bandwidth **(Target)**, 2-channel minimum |
| Function Generator | Signal injection testing | 1 Hz - 100 kHz **(Target)**, sine/square |
...
- Temperature: 20-25 degrees C **(Target)** (controlled room temperature)
...
2. Measure rod length using tape measure (should match specification +/-5 mm **(Target)**)
...
|------|-------------------|-----------|
| Ring 1 | 0.5 m **(Target)** | +/- 2 cm **(Target)** |
| Ring 2 | 1.5 m **(Target)** | +/- 2 cm **(Target)** |
| Ring 3 | 2.5 m **(Target)** | +/- 2 cm **(Target)** |
...
|-----------|--------|-----------|--------|
| 3.3V Rail | 3.3V **(Target)** | +/- 0.1V **(Target)** | [ ] Pass [ ] Fail |
| 5V Rail | 5.0V **(Target)** | +/- 0.1V **(Target)** | [ ] Pass [ ] Fail |
| Current Draw | < spec | See datasheet | [ ] Pass [ ] Fail |
| Voltage Stability | Stable | No fluctuations | [ ] Pass [ ] Fail |
| Temperature | < 50C **(Target)** | No excessive heat | [ ] Pass [ ] Fail |

: Power supply test criteria {#tbl-power-test}

### Test E2: Continuity and Shorts {#sec-test-e2}

With power off, measure resistance between power and ground (should be >100 ohm **(Target)**). Check for shorts between all signal lines. Verify all connections are continuous end-to-end. Confirm shield connections are proper.

### Test E3: Communication Interface {#sec-test-e3}

Connect probe to base hub. Send test command and verify response. Test data transfer with multiple packets. Check reliability over 100 transactions (must be >95% success rate **(Target)**).
...
|-----------|---------|---------|---------------|
| Inductance | Measured: ___ | Measured: ___ | 1-2 mH **(Target)** |
| Q Factor @ 10 kHz | Measured: ___ | Measured: ___ | $\geq$ 30 **(Target)** |
| DC Resistance | Measured: ___ | Measured: ___ | < 8 ohm **(Target)** |
| Coil Isolation | N/A | N/A | > 1 M ohm **(Target)** |
...
Configure DDS for 10 kHz test frequency. Measure output with oscilloscope. Verify frequency accuracy is within +/-1% **(Target)**. Check output amplitude stability. Measure THD (target: <1% **(Target)**). Test at 2, 5, 10, 20, and 50 kHz.

### Test MIT3: TX Driver Output {#sec-test-mit3}

Measure DDS output before driver. Measure driver output after amplification. Calculate gain and compare to design specification (+/-10% **(Target)**). Verify no clipping or distortion. Test across frequency range.

### Test MIT4: RX Chain Response {#sec-test-mit4}

Inject known test signal into RX input. Measure signal at each amplification stage. Calculate gain at each stage. Verify total system gain matches design (+/-10% **(Target)**). Measure noise floor (target: <1% of full scale **(Target)**).

### Test MIT5: Coupling Test {#sec-test-mit5}

Set up two probes 1-3 m apart. Place aluminum calibration target between probes. Configure TX on one probe, RX on other. Measure amplitude and phase response. Remove target and establish baseline. Expected response with target: 10-50% amplitude drop **(Target)**, 5-30 degree phase lag **(Target)**.

## ERT Subsystem Tests {#sec-ert-tests}

### Test ERT1: Ring Isolation {#sec-test-ert1}

Measure resistance between adjacent rings (should be >1 M ohm **(Target)**). Measure each ring to ground and to probe rod body (all should be >1 M ohm **(Target)**). Repeat measurements after water exposure to verify sealing integrity.

### Test ERT2: Contact Resistance {#sec-test-ert2}

Insert probe into test medium (sand box with known resistivity). Inject test current (0.5-1 mA **(Target)**). Measure voltage between adjacent rings. Calculate contact resistance (target: <1000 ohm **(Target)**). Verify readings are stable with no drift.

### Test ERT3: Measurement Chain {#sec-test-ert3}

Apply known voltage to ERT input. Select each ring via multiplexer. Verify correct ring selection. Measure at amplifier output. Read ADC value and compare to expected (should be within +/-5% **(Target)**). Test all rings sequentially.

### Test ERT4: Current Source {#sec-test-ert4}

Connect to known test load resistance. Configure current levels: 0.5, 1.0, 1.5, 2.0 mA **(Target)**. Measure actual current at each setting. Verify accuracy is within +/-5% **(Target)**. Test stability over 1 minute at each level.
...
Set up two probes (A and B) at fixed separation. Measure A transmitting to B. Measure B transmitting to A. Compare results - should match within 5% **(Target)**. Repeat for multiple probe pairs to verify consistency.

### Test INT3: Repeatability Test {#sec-test-int3}

Set up fixed test configuration. Take measurement. Wait 1 minute. Take same measurement. Repeat 5-10 times. Calculate mean and standard deviation. Pass criteria: standard deviation <5% of mean **(Target)**. No systematic drift over time.
...
Connect precision metal-film resistors (0.1% tolerance **(Manufacturer spec)**) to probe electrode inputs. Test values: 100 ohm, 1 k ohm, 10 k ohm **(Target)**. Inject currents at 0.5, 1.0, 2.0 mA **(Target)**. Record 50 samples for each combination. Calculate Mean Absolute Percentage Error (MAPE).

$$
\text{MAPE} = \frac{1}{n} \sum \frac{|R_{\text{measured}} - R_{\text{true}}|}{R_{\text{true}}} \times 100\%
$$

**Target:** MAPE < 1.0% **(Target)** across the dynamic range.

### MIT Sensitivity Validation (Standard Loop) {#sec-mit-validation}

Construct Standard Calibration Loop: single turn of 14 AWG copper wire, 10 cm diameter **(Target)**, shorted. Place loop coaxially with TX/RX pair at distances of 0.5 m, 1.0 m, 1.5 m **(Target)**. Measure change in mutual impedance at 10 kHz. Compare to analytical dipole-loop solution.

**Target:** Measurement within +/-10% **(Target)** of theoretical prediction.

### Receiver Noise Characterization {#sec-noise-characterization}

Short RX inputs at Zone Hub. Record 10-second timeseries at 30 kSPS **(Target)**. Compute Power Spectral Density (PSD). Report input-referred noise in nV/sqrt(Hz) at 2, 10, 50 kHz.

**Target:** < 20 nV/sqrt(Hz) **(Target)** (comparable to commercial geophysical amplifiers).
...
|----------|-------|----------------|--------------|
| Mechanical | M1-M4 | Rod integrity, sealing | 30 min **(Target)** |
| Electrical | E1-E3 | Power rails, communication | 30 min **(Target)** |
| MIT Subsystem | MIT1-MIT5 | Coil params, coupling | 60 min **(Target)** |
| ERT Subsystem | ERT1-ERT4 | Ring isolation, accuracy | 45 min **(Target)** |
| Integration | INT1-INT3 | Full system, reciprocity | 45 min **(Target)** |
| Environmental | ENV1-ENV2 | Waterproofing | 60 min **(Target)** |
