...
For a typical 16-probe array **(Target)**, this yields 240 unique measurements per acquisition cycle—each sampling a different geometric path through the subsurface.
...
|------------|-------------|----------------|
| X | Horizontal position (grid column) | 0 to 10 m **(Target)** |
| Y | Horizontal position (grid row) | 0 to 10 m **(Target)** |
| Z | Vertical depth below surface | 0 to -3 m **(Target)** (negative = below ground) |

: Probe coordinate system {#tbl-probe-coordinates}

Within each probe, multiple sensor rings are spaced along the shaft (e.g., at 0.5 m intervals **(Target)**). A 3 m probe **(Target)** with 0.5 m ring spacing **(Target)** contains **6 electrode rings**, each at a known depth. The complete array therefore samples a 3D volume defined by the probe positions (X, Y) and sensor depths (Z).
...
For a 16-probe array **(Target)** at 5 frequencies **(Target)**, this produces approximately 1,200 MIT measurements per acquisition cycle.
...
- **Units:** Always meters with two decimal places **(Target)**
