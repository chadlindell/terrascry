...
## Quantitative Validation Protocols (Scientific Verification)

To ensure the HIRT system meets scientific publication standards, rigorous quantitative validation is required beyond simple functional testing.

### 1. ERT Accuracy Validation (NIST-Traceable)

**Objective:** Quantify the absolute accuracy of the resistivity measurement chain.

**Protocol:**
1.  Connect a set of **precision metal-film resistors** (tolerance 0.1% or better) to the probe electrode inputs.
2.  Values to test: **100 &Omega;, 1 k&Omega;, 10 k&Omega;**.
3.  Inject currents: **0.5 mA, 1.0 mA, 2.0 mA**.
4.  Record 50 samples for each combination.

**Analysis:**
- Calculate the **Mean Absolute Percentage Error (MAPE)**:
  $$ MAPE = \frac{1}{n} \sum \left| \frac{R_{measured} - R_{true}}{R_{true}} \right| \times 100\% $$
- **Target:** MAPE < 1.0% across the dynamic range.

### 2. MIT Sensitivity Validation (Standard Loop)

**Objective:** Verify that the MIT magnetic coupling matches theoretical physics models.

**Protocol:**
1.  Construct a **Standard Calibration Loop**: A single turn of 14 AWG copper wire, 10cm diameter, shorted.
2.  Place the loop coaxially with the TX/RX pair at distances of **0.5m, 1.0m, 1.5m**.
3.  Measure the **Change in Mutual Impedance (&Delta;Z)** at 10 kHz.

**Analysis:**
- Compare measured signal vs. analytical solution for a dipole-loop system.
- **Target:** Measurement should align with theory within +/- 10% (accounting for loop geometry errors).

### 3. Receiver Noise Characterization

**Objective:** Determine the fundamental detection limit of the system.

**Protocol:**
1.  Short the RX inputs at the Zone Hub.
2.  Record a 10-second timeseries at 30 kSPS.
3.  Compute the **Power Spectral Density (PSD)**.

**Analysis:**
- Report the **Input-Referred Noise** in $nV / \sqrt{Hz}$ at key frequencies (2, 10, 50 kHz).
- **Target:** < 20 $nV / \sqrt{Hz}$ (comparable to commercial geophysical operational amps).
